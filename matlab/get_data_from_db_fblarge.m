%%
 ...
 %
 %%
function data = get_data_from_db_fblarge(config)
...
res = dbquery(['select a.photo_fbid, a.hyp_id, a.xmin, a.ymin, a.width,'...
    ...
    'a.hyp_id = p.hyp_id and p.version= ' num2str(config.FBLARGE_VERSION) ' limit 10000000']);
...
data.train_idx = find(mod(double(data.subject_id),10)~=1);
data.test_idx = find(mod(double(data.subject_id),10)==1);
data.val_idx = find(data.subject_id==0);
