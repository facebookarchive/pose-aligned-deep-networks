...
MODE = 'all';
if ~isequal(MODE,'all')
   views = load([config.TMP_DIR '/views.mat']);
   sel = getfield(views,MODE);
   level2_features_test = level2_features_test(sel,:);
   test_gfeatures = test_gfeatures(sel,:);
   test_labels = test_labels(sel,:);
end
fprintf('View mode: %s\n',MODE);
